...
spectra_process <- function(dataset_name) {
  cso_df <- read.csv(file.path(base_path,"splits_data","cross_split_overlap",
                               "spectra_tanimoto", 
                               paste0(dataset_name,"_spectra_tanimoto_cross_split_overlap.csv")))
  metrics_df <- read.csv(file.path(base_path,"results","chemprop_results",
                                   "spectra_tanimoto", "sheet", 
                                   paste0("spectra_ensemble_",dataset_name,".csv")))
...
rsu_process <- function(dataset_name, split_type) {
  ...
  metrics_df <- read.csv(file.path(base_path,"results","chemprop_results", split_type, 
                                   paste0(split_type, "_metrics_", dataset_name, ".csv")))
...
all_data <- all_data
write.csv(all_data, file = file.path(base_path, "results","regression_summary.csv"))
